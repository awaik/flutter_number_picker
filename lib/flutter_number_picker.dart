...
class FlutterNumberPicker extends StatelessWidget {
  ...
  bool _onDecimalNotification(Notification notification) {
    if (notification is ScrollNotification) {
      ...
      int indexOfMiddleElement = (horizontal)
          ? (notification.metrics.pixels + listViewWidth / 2) ~/ itemExtent
          : (notification.metrics.pixels + listViewHeight / 2) ~/ itemExtent;
...
class _NumberPickerDialogControllerState extends State<NumberPickerDialog> {
  ...
  FlutterNumberPicker _buildNumberPicker() {
    if (widget.decimalPlaces > 0) {
      return new FlutterNumberPicker.decimal(
      ...
      return new FlutterNumberPicker.integer(
